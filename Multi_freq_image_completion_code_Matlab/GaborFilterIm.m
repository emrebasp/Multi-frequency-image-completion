function result = GaborFilterIm(img, lambda, sigma, theta, phi, gamma, bandwidth)
...
filterkernel = GaborKernelIm2D(lambda, sigma, theta(1), phi(1), gamma, bandwidth); % called only to determine the size of the filterkernel 
...
while (cnt1 <= nrtheta)
  ...
  while (cnt2 <= nrphi) % for each phi the convolution is calculated
    filterkernel = GaborKernelIm2D(lambda, sigma, theta(cnt1), phi(cnt2), gamma, bandwidth);
