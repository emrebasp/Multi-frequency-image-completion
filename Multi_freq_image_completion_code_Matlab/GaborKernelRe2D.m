function result = GaborKernelRe2D(lambda, sigma, theta, phi, gamma, bandwidth)
...
resultRe = a*exp(-b*(xp.*xp + gamma2*(yp.*yp))) .* cos(-f*yp - phi);
...
% % % end
% % % 
% % % result(pnegIm) = posIm*resultIm(pnegIm);
